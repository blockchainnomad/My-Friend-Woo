...
function WooList() {
    ...
    const wooNft = useContext(Context).wooNft;
    ...
    const next = localStorage.getItem("WOO.next");
    localStorage.setItem("WOO.next", startIndex + ITEMS_PER_PAGE);
    ...
    useEffect(() => {
        ...
        if (wooNft !== null) {
            ...
            console.log(`NFT contract address=${wooNft.address}`);

            const wooNftInterface = new ethers.utils.Interface(artifact.contracts.Woo.abi);

            defaultProvider.on({ address: wooNft.address }, (logs) => {
                const result = wooNftInterface.parseLog(logs);
    ...
    const handleMint = async (e) => {
        ...
        if (wooNft !== null) {
            ...
            const tx = await wooNft.mint(OWNER, tokenId, tokenURI, { gasLimit: 3000000 });
            try {
                await tx.wait();
    ...
    const handleSale = async (e) => {
        if (wooNft !== null) {
            ...
            let m = "";
            const tokenId = e.target.getAttribute("tokenid");
            const asset = await seaport.api.getAsset({ tokenAddress: wooNft.address, tokenId });
