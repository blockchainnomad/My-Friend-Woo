                ...
                if (result != null) {
                    fetch(result[0].slice(result[0].indexOf("=") + 1))
                        ...
                        .then(data => {
                            ...
                            resolve({
                                "image": data.image.toString().replaceAll("ipfs://", "https://dweb.link/ipfs/"),
...
const saveMintedItems = (tokenId) => {
    ...
    const previousMinted = localStorage.getItem("WOO.minted");

    if (previousMinted === null) {
        localStorage.setItem("WOO.minted", tokenId);
    } else {
        const mintedArray = previousMinted.toString().split(",");
        if (mintedArray.indexOf(tokenId.toString()) < 0) {
            localStorage.setItem("WOO.minted", previousMinted + "," + tokenId);
        }
    }
